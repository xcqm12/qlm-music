 ...
 * @version 7.1.2-ultimate-fix-v3.1
 * @author 整合优化版（api.qlm.org.cn 专项修复，仅限网易云）
...
(function() {
    ...
    // ---------- qorg（api.qlm.org.cn 专项修复：仅限网易云，搜索自动补全ID）----------
    async function qorgGetMusicUrl(platform, songInfo, quality) {
        // 平台限制：api.qlm.org.cn 只支持网易云（netease）和自建网易云
        ...
        // 3. 映射 source 参数（自建网易云同样使用 netease）
    ...
    const SOURCE_HANDLERS = [
        ...
        new SourceHandler('qorg', qorgGetMusicUrl, 3, { supportedPlatforms: ['wy','wycloudmusic'] }), // api.qlm.org.cn 仅限网易云
            ...
            for (const handler of chain) {
                try {
                    ...
                    if (['汽水VIP','网易云盘','自建网易云','Free listen 酷我','Free listen 酷狗','Free listen 网易云'].includes(handler.name)) {
                        ...
                        // qorg 需要 platform、songInfo 和 quality
    ...
    async function qorgSearch(keyword, page, pageSize) {
        ...
        if (res?.code===200 && res.data) {
            const list = (res.data.list||[]).map(item => ({
                ...
                albumName: item.album, duration: item.duration, pic: item.pic||item.cover, _source: 'wy' // 标记为网易云
            }));
    ...
    async function initialize() {
        ...
        console.log('[聚合音源 v7.1.2-ultimate-fix-v3.1] 初始化 (api.qlm.org.cn 仅限网易云)');
        startStatsCleanup();
        setupEventListener();
