...
| 版本 | 日期 | 更新内容 |
|-----|------|---------|
| **qlm-v1.0.0** | 2026 | 🎉 **重构版 1.0.0**，基于 v7.0.7 架构重构，整合 v9.0.9 全部 28+ 音源；修复初始化失败问题；修复 not a function 错误；修复 Network request failed 问题；修复 星海备缺少歌曲ID 问题；**修复 qorg 缺少歌曲ID问题**（增加搜索补全功能，ID缺失时自动通过搜索接口补全）；添加缺失的音源处理函数（肥猫不肥、梓澄qwq、梓澄公益2代、聚合API、野花野草、Meting、Free listen、独家音源）；添加 PLATFORM_TO_SOURCE 配置；添加 httpFetch 工具函数 |
