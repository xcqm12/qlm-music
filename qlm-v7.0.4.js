...
(function() {
    ...
    const API_ENDPOINTS = {
        ...
        qorg: {
            ...
            endpoints: {
                ...
                search: "/music/search",
                lyric: "/music/lyric",
                wycloud: "/wycloud",  // 网易云盘接口
                wycloudCookie: "/wycloud/cookie"  // Cookie 设置接口
            }
        },
        xinghaiMain: {
    ...
    const state = {
        ...
        ikunChecked: false,
        cleanupTimer: null,
        wycloudCookie: null,  // 网易云盘 Cookie
        wycloudCookieExpire: 0  // Cookie 过期时间
    };
    ...
    /**
     * 自动获取网易云盘 Cookie（通过提示用户输入或从API获取）
     */
    async function autoGetWycloudCookie() {
        ...
        console.log('[网易云盘] 获取方式: 登录网页版网易云音乐后，在开发者工具中复制Cookie');
    ...
    async function setWycloudCookie(cookie) {
        ...
        try {
            ...
            return true; // 即使验证失败也保存，可能是API问题
        }
    }
  ...
  async function getUrlWithFallback(platform, songInfo, quality) {
    ...
    const platformQualityMap = {
        ...
        wycloud: ['128k', '320k', 'flac', 'flac24bit']  // 新增网易云盘
    };
    
    const platformNames = {
        tx: 'QQ音乐', wy: '网易云音乐', kw: '酷我音乐',
        kg: '酷狗音乐', mg: '咪咕音乐', qishui: '汽水VIP',
        qorg: 'qorg音源', wycloud: '网易云盘'  // 新增
    };
    ...
    async function initialize() {
        ...
        console.log('[聚合音源·超级整合版（完整版）] v7.0.4 初始化中...');
        console.log(`[聚合音源] 开源地址: ${ANNOUNCEMENT.repo}`);
        console.log(`[聚合音源] 交流群: ${ANNOUNCEMENT.qqGroup}`);
        console.log('[聚合音源] 已集成音源: ikun, 聚合API, qorg, 星海, 溯音, 六音, 独家音源, 长青SVIP, 念心SVIP, 野花野草, Meting, 汽水VIP, 肥猫, 肥猫不肥, 梓澄公益, 梓澄qwq, 梓澄公益2代, 無名, 小熊猫, Free listen, 网易云盘');
        console.log('[聚合音源] 网易云盘功能已启用，支持搜索和播放云盘歌曲');
        ...
        console.log('[聚合音源·超级整合版（完整版）] v7.0.4 已加载');
