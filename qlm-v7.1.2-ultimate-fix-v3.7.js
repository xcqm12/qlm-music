 ...
 * @description 整合 ikun/GD音乐台/野草/长青SVIP/Free listen 等音源，完善错误处理，支持 weapi/eapi 双加密，qorg 三重回退，终极兜底 api.qlm.org.cn，失败自动跳歌
 * @version 7.1.2-ultimate-fix-v3.7
...
(function() {
    ...
    function extractUrl(data, sourceName) {
        ...
        if (SafeUtils.isObject(data)) {
            const url = data.url || data.download_url || data.source_url || data.play_url;
    ...
    class SourceHandler {
        constructor(name, fn, priority, opts={}) {
            ...
            this.needUrlValidation = opts.needUrlValidation !== false; // 默认开启URL可达性预检
        }
    ...
    function createMusicUrlFetcher(apiUrl, apiKey, name) {
        return async function(source, musicInfo, quality) {
            ...
            switch(body.code) {
                case 0: {
                    const url = extractUrl(body.data, name);
                    if (url) return url;
                    throw new Error(name+': data中无有效URL');
                }
    ...
    async function juheGetMusicUrl(source, info) {
        ...
        try {
            ...
            if (metingServer) {
                ...
                if (id) {
                    ...
                    const resp = await httpRequestWithRetry(url, {
                        method: 'GET',
                        timeout: CONFIG.REQUEST_TIMEOUT,
                        headers: { 'User-Agent': `lx-music-${env}/${version}` }
                    }, 1);
                    const res = resp.body;
                    const finalUrl = extractUrl(res?.url || res?.data?.url || (typeof res === 'string' ? res : null), '聚合API(Meting)');
        ...
        try {
            ...
            // 辅助提取：先常见字段，再深度递归
            const extractUrlFromBody = (data, label) => {
                if (!data) return null;
                if (SafeUtils.isString(data) && HTTP_REGEX.test(data)) return data;
                const direct = extractUrl(data, label);
                if (direct) return direct;
                const deep = deepExtractUrl(data);
                if (deep && HTTP_REGEX.test(deep)) return deep;
                return null;
            };

            // 状态码 200
            if (body.code === 200) {
                const url = extractUrlFromBody(body.data, 'juhe') || extractUrlFromBody(body, 'juhe');
                if (url) return validateUrl(url, 'juhe');
            }

            // 状态码 303
            if (body.code === 303) {
                let data = typeof body.data === 'string' ? JSON.parse(body.data) : body.data;
            ...
            // 兜底：直接从 body 中递归提取 URL
            const fallbackUrl = extractUrlFromBody(body, 'juhe');
            if (fallbackUrl) return validateUrl(fallbackUrl, 'juhe');
    ...
    async function xinghaiMainGetUrl(platform, songId, quality, songInfo) {
        ...
        // 深度提取URL
        const url = extractUrl(res) || deepExtractUrl(res);
    ...
    async function grassGetMusicUrl(platform, songId, quality, songInfo) {
        ...
        const url = extractUrl(res?.url || res, '野草');
        if (url) return url;
    ...
    async function suyinQQGetUrl(songInfo, quality) {
        ...
        for (const br of brList) {
            try {
                ...
                const url = extractUrl(res?.music || res?.url || res, '溯音QQ');
                if (url) return url;
            } catch(e) { lastErr = e; }
        }
    ...
    async function suyinWyGetUrl(songInfo) {
        ...
        if (res?.code===0 && res?.data) {
            ...
            const url = extractUrl(item?.url || item, '溯音网易云');
            if (url) return url;
        }
    ...
    async function suyinKwGetUrl(songInfo) {
        ...
        const url = extractUrl(res?.data?.url || res?.url || res, '溯音酷我');
        if (url) return url;
    ...
    async function suyinMgGetUrl(songInfo) {
        ...
        if (res?.code===200 && res?.musicInfo) {
            const url = extractUrl(res.musicInfo, '溯音咪咕');
            if (url) return url;
        }
    ...
    async function sixyinGetUrl(platform, songId, quality, songInfo) {
        ...
        const url = extractUrl(res?.url || res, '六音');
    ...
    async function flowerGrassGetUrl(platform, songId, quality) {
        ...
        for (const base of [API_ENDPOINTS.flower, API_ENDPOINTS.grass?.base].filter(Boolean)) {
            try {
                ...
                const url = extractUrl(res?.url || res, '野花野草');
                if (url) return url;
            } catch(e) {}
        }
    ...
    async function metingGetUrl(platform, songId, quality) {
        ...
        const finalUrl = extractUrl(res?.url || res, 'Meting');
    ...
    // ---------- Free listen 酷我 / 酷狗 (复用肥猫音源) ----------
    async function freelistenKwGetMusicUrl(songInfo, quality) {
        return feimaoGetMusicUrl('kw', songInfo, quality);
    }
    async function freelistenKgGetMusicUrl(songInfo, quality) {
        return feimaoGetMusicUrl('kg', songInfo, quality);
    }
    ...
    const SOURCE_HANDLERS = [
        ...
        new SourceHandler('长青SVIP', changqingGetUrl, 12, { supportedPlatforms: ['tx','wy','kw','kg','mg'], needUrlValidation: true }),
        new SourceHandler('念心SVIP', nianxinGetUrl, 13, { supportedPlatforms: ['tx','wy','kw','kg','mg'], needUrlValidation: true }),
    ...
    async function getUrlWithFallback(platform, songInfo, quality) {
        ...
        if (!chain.length) {
            // 如果没有常规处理器，直接走兜底
            const fallbackUrl = await ultimateFallback(platform, songInfo, q, cacheKey);
        ...
        const handler = async ({ action, source, info }) => {
            ...
            try {
                ...
                if (action === 'musicUrl') {
                    ...
                    const url = await getUrlWithFallback(platform, musicInfo, info?.type||'320k');
                    if (!url) {
                        console.warn('[聚合音源] 所有音源均失败，返回空字符串以跳过当前歌曲');
                        return ''; // 空字符串让LX自动跳到下一首
                    }
                    return url;
                }
    ...
    async function initialize() {
        ...
        console.log('[聚合音源 v7.1.2-ultimate-fix-v3.7] 初始化 (qorg 三重回退 + 终极兜底自动跳歌)');
        startStatsCleanup();
        setupEventListener();
