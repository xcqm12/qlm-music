...
(function() {
    ...
    async function qorgGetMusicUrl(platform, songInfo, quality) {
        ...
        if (!sid && platform === 'tx') sid = (meta.qq || {}).mid || (meta.qq || {}).songid || meta.mid;
        if (!sid && platform === 'wy') sid = meta.songmid || meta.id || info.songmid;

        // 2. 新增：如果ID仍然为空，尝试通过Qorg搜索接口自动补全（基于 api.qlm.org.cn）
        if (!sid) {
            const keyword = info.name || info.title || '';
            const singer  = info.singer || info.artist || '';
            if (keyword) {
                console.warn(`[qorg] 歌曲ID缺失，尝试通过 API 搜索补全: ${keyword} ${singer}`);
                try {
                    // 调用本文件内部的 qorgSearch，它能直接使用搜索缓存
                    ...
                    if (searchRes && searchRes.list && searchRes.list.length > 0) {
                        ...
                        if (sid) {
                            console.log(`[qorg] 搜索获取到 ID: ${sid} (${match.name} - ${match.singer})`);
                        }
                    }
                } catch (e) {
                    console.warn('[qorg] 搜索补全 ID 失败:', e.message);
                }
            }
        }
    ...
    const SOURCE_HANDLERS = [
        ...
        new SourceHandler('qorg', qorgGetMusicUrl, 3, { supportedPlatforms: ['tx','wy','kw','kg','mg'] }),
        ...
        const handler = async ({ action, source, info }) => {
            ...
            try {
                ...
                if (source === 'qorg') {
                    ...
                    if (action === 'musicUrl') return await qorgGetMusicUrl(source, info.musicInfo, info.type);
                }
                if (action === 'musicUrl') {
    ...
    async function initialize() {
        ...
        console.log('[聚合音源 v7.1.2-ultimate-fix-v2] 初始化，支持 weapi/eapi 双加密，qorg 已支持搜索补全ID');
        startStatsCleanup();
        setupEventListener();
