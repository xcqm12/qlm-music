 ...
 * @version 7.1.2-ultimate-fix-v3
 * @author 整合优化版（含 qorg ID 修复）
...
(function() {
    ...
    async function qorgGetMusicUrl(platform, songInfo, quality) {
        ...
        // 2. 搜索补全：仅对网易云平台启用（api.qlm.org.cn 仅支持网易云搜索）
        if (!sid && (platform === 'wy' || platform === 'wycloudmusic')) {
            ...
            for (const handler of chain) {
                try {
                    ...
                    if (['汽水VIP','网易云盘','自建网易云','Free listen 酷我','Free listen 酷狗','Free listen 网易云'].includes(handler.name)) {
                        ...
                        // 修复：qorg 需要完整的 platform 和 songInfo
        ...
        const handler = async ({ action, source, info }) => {
            ...
            try {
                ...
                if (source === 'qorg') {
                    ...
                    if (action === 'musicUrl') {
                        // 修复：从 musicInfo._source 提取真实平台，默认网易云
    ...
    async function initialize() {
        ...
        console.log('[聚合音源 v7.1.2-ultimate-fix-v3] 初始化 (qorg ID 修复 + 参数修正)');
        startStatsCleanup();
        setupEventListener();
