 ...
 * @version 7.0.1 - 完整整合版（修复musicUrl支持）
...
(function() {
    ...
    const handleBase64Encode = (data) => {
        try {
            if (utils && utils.buffer && utils.buffer.from && utils.buffer.bufToString) {
                var bufData = utils.buffer.from(data, 'utf-8');
                return utils.buffer.bufToString(bufData, 'base64');
            }
    ...
    const PLATFORM_TO_SOURCE = {
        tx: { main: "tencent", backup: "qq", meting: "tencent", ikun: "tx", dusiyinyuan: "qq" },
        wy: { main: "netease", backup: "netease", meting: "netease", ikun: "wy", dusiyinyuan: "wy" },
        kw: { main: "kuwo", backup: "kuwo", ikun: "kw", dusiyinyuan: "kw" },
        kg: { main: "kugou", ikun: "kg", dusiyinyuan: "kg" },
        mg: { main: "migu", ikun: "mg", dusiyinyuan: "mg" },
        sixyin: { main: "sixyin" },
        local: { main: "local" }
    };
    ...
    // 無名音源的加密函数
    function wumingAesEncrypt(data, key, iv, mode) {
    ...
    async function httpRequestWithRetry(url, options = {}, retries = CONFIG.MAX_RETRIES) {
        let lastError = null;
    ...
    async function httpGet(url, params = {}, timeout = CONFIG.REQUEST_TIMEOUT) {
    ...
    async function httpPost(url, body = {}, timeout = CONFIG.REQUEST_TIMEOUT) {
    ...
    async function httpGetWithFallback(urls, params = {}, timeout = CONFIG.REQUEST_TIMEOUT) {
    ...
    function stopStatsCleanup() {
        if (state.cleanupTimer) {
            safeClearTimeout(state.cleanupTimer);
    ...
    // ==================== 新增 qorg API 处理器（未启用） ====================
    // 标注：此 API 暂未启用，保留供后续使用
    async function qorgGetMusicUrl(platform, songInfo, quality) {
        if (!CONFIG.QORG_ENABLED) {
            throw new Error('qorg API 暂未启用');
        }
        ...
        switch (platform) {
            ...
            default: throw new Error(`qorg 不支持平台: ${platform}`);
        }
    ...
    async function qorgSearch(keyword, page = 1, pageSize = 30) {
        if (!CONFIG.QORG_ENABLED) {
    ...
    const SOURCE_HANDLERS = {
        ...
        xinghaiMain: { name: '星海主', fn: xinghaiMainGetUrl, priority: 3, timeout: 12000 },
        xinghaiBackup: { name: '星海备', fn: xinghaiBackupGetUrl, priority: 4, timeout: 12000 },
        suyin: { name: '溯音', fn: suyinGetUrl, priority: 5, timeout: 15000 },
        sixyin: { name: '六音', fn: sixyinGetUrl, priority: 6, timeout: 12000 },
        dusiyinyuan: { name: '独家音源', fn: dusiyinyuanGetUrl, priority: 7, timeout: 15000 },
        changqing: { name: '长青SVIP', fn: changqingGetUrl, priority: 8, timeout: 10000 },
        nianxin: { name: '念心SVIP', fn: nianxinGetUrl, priority: 9, timeout: 10000 },
        flowerGrass: { name: '野花野草', fn: flowerGrassGetUrl, priority: 10, timeout: 10000 },
        meting: { name: 'Meting', fn: metingGetUrl, priority: 11, timeout: 10000 },
        qishui: { name: '汽水VIP', fn: qishuiGetUrl, priority: 12, timeout: 20000 },
        feimao: { name: '肥猫', fn: feimaoGetMusicUrl, priority: 13, timeout: 12000, requireSource: true },
        feimaobufei: { name: '肥猫不肥', fn: feimaobufeiGetMusicUrl, priority: 14, timeout: 12000, requireSource: true },
        zicheng: { name: '梓澄公益', fn: zichengGetMusicUrl, priority: 15, timeout: 12000, requireSource: true },
        zichengqwq: { name: '梓澄qwq', fn: zichengqwqGetMusicUrl, priority: 16, timeout: 12000, requireSource: true },
        zicheng2: { name: '梓澄公益2代', fn: zicheng2GetMusicUrl, priority: 17, timeout: 12000, requireSource: true },
        wuming: { name: '無名', fn: wumingGetMusicUrl, priority: 18, timeout: 15000, requireSource: true },
        xiaoxiongmao: { name: '小熊猫', fn: xiaoxiongmaoGetMusicUrl, priority: 19, timeout: 15000, requireSource: true },
        freelisten: { name: 'Free listen', fn: freelistenGetMusicUrl, priority: 20, timeout: 15000, requireSource: true },
        // 新增 qorg 处理器（未启用，优先级最低）
        qorg: { name: 'qorg(未启用)', fn: qorgGetMusicUrl, priority: 99, timeout: 12000, requireSource: false }
    };
    ...
    function buildSourceChain(platform) {
        ...
        // 添加所有支持该平台的处理器
        const handlerOrder = [
            'ikun', 'juhe', 'xinghaiMain', 'xinghaiBackup', 'suyin', 'sixyin', 
        ...
        for (const handlerName of handlerOrder) {
            ...
            if (handler) {
                ...
                if (handlerName === 'qorg' && !['tx', 'wy', 'kw', 'kg', 'mg'].includes(platform)) supportsPlatform = false;
    ...
    async function getUrlWithFallback(platform, songInfo, quality) {
        if (!platform || (!safeIncludes(PLATFORMS, platform) && platform !== 'qishui' && platform !== 'qorg')) {
        ...
        // qorg 平台处理（如果启用）
        if (platform === 'qorg' && CONFIG.QORG_ENABLED) {
            const promise = (async () => {
                try {
    ...
    // 如果 qorg 启用，添加到配置中
    if (CONFIG.QORG_ENABLED) {
        platformQualityMap.qorg = ['128k', '320k', 'flac', 'flac24bit'];
        platformNames.qorg = 'qorg(未启用)';
    }
    ...
    function setupEventListener() {
        const handleRequest = async ({ action, source, info }) => {
            ...
            return new Promise(async (resolve, reject) => {
                try {
                    // 处理 qishui 和 qorg 的特殊操作
                    if (source === 'qishui') {
                        const result = await qishuiHandler(action, info);
                        return resolve(result);
                    }
                    
                    if (source === 'qorg' && CONFIG.QORG_ENABLED) {
                        if (action === 'musicSearch' || action === 'search') {
    ...
    async function initialize() {
        ...
        console.log('[聚合音源·超级整合版（完整版）] v7.0.1 初始化中...');
        console.log('[聚合音源] 已集成音源: ikun, 聚合API, 星海, 溯音, 六音, 独家音源, 长青SVIP, 念心SVIP, 野花野草, Meting, 汽水VIP, 肥猫, 肥猫不肥, 梓澄公益, 梓澄qwq, 梓澄公益2代, 無名, 小熊猫, Free listen');
        
        if (CONFIG.QORG_ENABLED) {
            console.log('[聚合音源] 新增 API: qorg (已启用)');
        } else {
            console.log('[聚合音源] 新增 API: qorg (未启用，标注备用)');
        }
        ...
        console.log('[聚合音源·超级整合版（完整版）] v7.0.1 已加载');
