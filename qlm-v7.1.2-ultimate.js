 ...
 * @description 整合 ikun/GD音乐台/野草/长青SVIP/Free listen 等音源，完善错误处理
 * @version 7.1.2-ultimate
 * @author 整合优化版
...
(function() {
    ...
    const CONFIG = Object.freeze({
        ...
        // GD音乐台 (星海API)
        XINGHAI_MAIN_API_URL: "https://music-api.gdstudio.xyz/api.php",
        
        // 野草音源 (仅酷我)
        GRASS_API_URL: "http://grass.tempmusic.ss.tk/v1",
        
        // 长青SVIP
        CHANGQING_API_URL: "http://175.27.166.236",
        CHANGQING_UPDATE_URL: "https://13413.kstore.vip/lxmusic/changqing.json",
        
        // Free listen 源
        FREELISTEN_KW_ENABLED: true,
        FREELISTEN_KG_ENABLED: true,
        FREELISTEN_WY_ENABLED: true,
    ...
    const API_ENDPOINTS = Object.freeze({
        ikun: { url: "https://api.ikunshare.com/url", hkUrl: "https://songapi.ikunshare.link/url", script: "https://api.ikunshare.com/script/lxmusic" },
        ...
        grass: { base: "http://grass.tempmusic.ss.tk/v1", endpoints: { url: "/url" } },
        suyin: {
        ...
        freelisten: {
            kw: { base: "http://www.kuwo.cn/api/v1/www/music/playUrl", cookieUrl: "http://www.kuwo.cn/" },
            kg: { base: "https://wwwapi.kugou.com/yy/index.php?r=play/getdata" },
            wy: { base: "https://interface3.music.163.com/eapi/song/enhance/player/url" }
        },
        nianxin: { 
    ...
    async function ikunGetMusicUrl(source, musicInfo, quality) {
        ...
        const urls = [
            ...
            `${CONFIG.IKUN_HK_API_URL}/url/${source}/${songId}/${quality || '320k'}`
        ];
        
        let lastError = null;
        for (const url of urls) {
            try {
                const res = await httpFetch(url, { method: "GET", headers, follow_max: 5 });
                const body = res.body;
                if (!body || isNaN(Number(body.code))) continue;
                
                switch (body.code) {
                    case 200: return body.url || body.data;
        ...
        throw lastError || new Error('ikun: 所有服务器均失败');
    }
    ...
    async function xinghaiMainGetUrl(platform, songId, quality, songInfo) {
        ...
        if (!res || typeof res !== 'object') throw new Error(res?.message || '星海主API未返回有效响应');
    ...
    async function grassGetMusicUrl(platform, songId, quality, songInfo) {
        if (platform !== 'kw') throw new Error('野草音源仅支持酷我音乐');
        
        const id = songId || getSongId(platform, songInfo);
        if (!id) throw new Error('野草音源: 缺少歌曲ID');
        
        const userAgent = env ? `lx-music-${env}/${version}` : `lx-music-request/${version}`;
        
        try {
            const res = await httpGet(`${API_ENDPOINTS.grass.base}/url/${platform}/${id}/${quality || '320k'}`, {}, CONFIG.REQUEST_TIMEOUT);
            
            if (res && res.url) return res.url;
            if (res && res.data) return res.data;
            throw new Error('野草音源未返回URL');
        } catch (e) {
            throw new Error(`野草音源失败: ${e.message}`);
        }
    }
    ...
    async function flowerGrassGetUrl(platform, songId, quality, songInfo) {
        ...
        const sources = [API_ENDPOINTS.flower, API_ENDPOINTS.grass && API_ENDPOINTS.grass.base].filter(Boolean);
        let lastError = null;
    ...
    // ==================== Free listen - 酷我音乐 ====================
    function freelistenKwEncrypt(str, pwd) {
        ...
        return new Promise((resolve, reject) => {
            ...
            httpFetch('http://www.kuwo.cn/', {
                ...
                if (result) {
                    ...
                    if (index > -1) {
                        httpFetch(result[index], {
                            headers: {
                                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:82.0) Gecko/20100101 Firefox/82.0',
                                Referer: 'http://www.kuwo.cn/',
                            },
                        }).then(resp => {
                            const match = resp.body.match(/Hm_Iuvt_(\w+)/);
                            if (match) {
                                resolve(freelistenKwEncrypt(token, match[0]));
                            } else resolve(freelistenKwEncrypt(token, defaultKey));
                        }).catch(() => resolve(freelistenKwEncrypt(token, defaultKey)));
                    } else {
                        resolve(freelistenKwEncrypt(token, defaultKey));
                    }
                } else {
                    resolve(freelistenKwEncrypt(token, defaultKey));
                }
    ...
    async function freelistenKwGetMusicUrl(songInfo, quality) {
        const songmid = songInfo.songmid || songInfo.id;
        if (!songmid) throw new Error('Free listen 酷我: 缺少歌曲ID');
        
        const qualityMap = { '128k': '128kmp3', '320k': '320kmp3' };
        const br = qualityMap[quality] || '128kmp3';
        
        if (!state.kwToken) {
            state.kwToken = await freelistenKwGetToken();
        }
        
        return new Promise((resolve, reject) => {
            const target_url = `http://www.kuwo.cn/api/v1/www/music/playUrl?mid=${songmid}&type=music&br=${br}`;
            httpFetch(target_url, {
                method: 'GET',
                headers: {
                    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:82.0) Gecko/20100101 Firefox/82.0',
                    Referer: 'http://kuwo.cn/',
                    Secret: state.kwToken,
                    cookie: state.kwCookie,
                },
            }).then(resp => {
                if (resp.body.code != 200) return reject(new Error('Free listen 酷我: 获取失败'));
    ...
    // ==================== Free listen - 酷狗音乐 ====================
    async function freelistenKgGetMusicUrl(songInfo, quality) {
        const hash = songInfo.hash;
        const albumId = songInfo.albumId || '';
        if (!hash) throw new Error('Free listen 酷狗: 缺少hash');
        ...
        return new Promise((resolve, reject) => {
            ...
            httpFetch(target_url, { method: 'GET' }).then(resp => {
                const data = resp.body;
                if (data.status !== 1) return reject(new Error('Free listen 酷狗: 获取失败 - ' + data.err_code));
                if (data.data.privilege > 9) return reject(new Error('Free listen 酷狗: 无权限'));
                resolve(resp.body.data.play_backup_url || resp.body.data.play_url);
            }).catch(err => reject(err));
        });
    }
    ...
    async function freelistenWyGetMusicUrl(songInfo, quality) {
        ...
        const data = freelistenWyEapi(eapiUrl, d);
        
        let cookie = 'os=pc';
        if (CONFIG.NETEASE_CLOUD_COOKIE_KEY) {
            cookie = CONFIG.NETEASE_CLOUD_COOKIE_KEY + '; ' + cookie;
        }
    ...
    const SOURCE_HANDLERS = [
        ...
        new SourceHandler('星海主(GD音乐台)', xinghaiMainGetUrl, 6, { timeout: 15000, supportedPlatforms: ['tx', 'wy', 'kw'] }),
        new SourceHandler('星海备', xinghaiBackupGetUrl, 7, { timeout: 15000 }),
        new SourceHandler('野草(酷我)', grassGetMusicUrl, 8, { timeout: 15000, supportedPlatforms: ['kw'] }),
        new SourceHandler('溯音', suyinGetUrl, 9, { timeout: 15000, supportedPlatforms: ['tx', 'wy', 'kw', 'mg'] }),
        new SourceHandler('六音', sixyinGetUrl, 10, { timeout: 15000, supportedPlatforms: ['tx', 'wy', 'kw', 'kg', 'mg'] }),
        new SourceHandler('独家音源', dusiyinyuanGetUrl, 11, { timeout: 15000, supportedPlatforms: ['tx', 'wy', 'kw', 'kg', 'mg'] }),
        new SourceHandler('长青SVIP', changqingGetUrl, 12, { timeout: 15000, supportedPlatforms: ['tx', 'wy', 'kw', 'kg', 'mg'] }),
        new SourceHandler('念心SVIP', nianxinGetUrl, 13, { timeout: 15000, supportedPlatforms: ['tx', 'wy', 'kw', 'kg', 'mg'] }),
        new SourceHandler('野花野草', flowerGrassGetUrl, 14, { timeout: 15000 }),
        new SourceHandler('Meting', metingGetUrl, 15, { timeout: 15000, supportedPlatforms: ['tx', 'wy'] }),
        new SourceHandler('汽水VIP', qishuiGetUrl, 16, { timeout: 20000 }),
        new SourceHandler('Free listen 酷我', freelistenKwGetMusicUrl, 17, { timeout: 15000, supportedPlatforms: ['kw'] }),
        new SourceHandler('Free listen 酷狗', freelistenKgGetMusicUrl, 18, { timeout: 15000, supportedPlatforms: ['kg'] }),
        new SourceHandler('Free listen 网易云', freelistenWyGetMusicUrl, 19, { timeout: 15000, supportedPlatforms: ['wy'] }),
        new SourceHandler('肥猫', feimaoGetMusicUrl, 20, { requireSource: true, timeout: 15000, supportedPlatforms: ['tx', 'wy', 'kw', 'kg', 'mg'] }),
        new SourceHandler('肥猫不肥', feimaobufeiGetMusicUrl, 21, { requireSource: true, timeout: 15000, supportedPlatforms: ['tx', 'wy', 'kw', 'kg', 'mg'] }),
        new SourceHandler('梓澄公益', zichengGetMusicUrl, 22, { requireSource: true, timeout: 15000, supportedPlatforms: ['tx', 'wy', 'kw', 'kg', 'mg'] }),
        new SourceHandler('梓澄qwq', zichengqwqGetMusicUrl, 23, { requireSource: true, timeout: 15000, supportedPlatforms: ['tx', 'wy', 'kw', 'kg', 'mg'] }),
        new SourceHandler('梓澄公益2代', zicheng2GetMusicUrl, 24, { requireSource: true, timeout: 15000, supportedPlatforms: ['tx', 'wy', 'kw', 'kg', 'mg'] })
    ];
        ...
        const promise = (async () => {
            ...
            for (const handler of chain) {
                try {
                    ...
                    } else if (['汽水VIP', '网易云盘', '自建网易云', 'Free listen 酷我', 'Free listen 酷狗', 'Free listen 网易云'].includes(handler.name)) {
    ...
    async function initialize() {
        ...
        console.log('[聚合音源·超级整合版（终极修复版）+ 自建网易云] v7.1.2-ultimate 初始化中...');
        console.log(`[聚合音源] 开源地址: ${ANNOUNCEMENT.repo}`);
        console.log(`[聚合音源] 交流群: ${ANNOUNCEMENT.qqGroup}`);
        console.log('[聚合音源] 已集成: ikun, GD音乐台(星海), 野草, 长青SVIP, Free listen, 聚合API, qorg, 溯音, 六音, 独家音源, 念心SVIP, 野花野草, Meting, 汽水VIP, 肥猫系列, 梓澄系列, 网易云盘, 自建网易云');
        ...
        console.log('[聚合音源·超级整合版（终极修复版）+ 自建网易云] v7.1.2-ultimate 已加载');
