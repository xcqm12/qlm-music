 ...
 * @version 7.1.2-ultimate-fix-v3.3
 * @author 整合优化版（qorg 改用 /song/url 端点，彻底修复 Bad Request）
...
(function() {
    ...
    // ---------- qorg（彻底修复：改用 /song/url 端点，不再使用签名接口）----------
    async function qorgGetMusicUrl(platform, songInfo, quality) {
        ...
        throw new Error('qorg: 获取URL失败 (' + (res?.msg || res?.message || '无数据') + ')');
    }
    ...
    async function initialize() {
        ...
        console.log('[聚合音源 v7.1.2-ultimate-fix-v3.3] 初始化 (qorg 已修复)');
        startStatsCleanup();
        setupEventListener();
