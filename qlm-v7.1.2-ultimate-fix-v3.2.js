 ...
 * @description 整合 ikun/GD音乐台/野草/长青SVIP/Free listen 等音源，完善错误处理，支持 weapi/eapi 双加密
 * @version 7.1.2-ultimate-fix-v3.2
 * @author 整合优化版（api.qlm.org.cn 获取失败修复 + 回退端点）
...
(function() {
    ...
    const PLATFORM_TO_SOURCE = Object.freeze({
        tx: { main: "tencent", ikun: "tx", dusiyinyuan: "qq" },
        wy: { main: "netease", ikun: "wy", dusiyinyuan: "wy" },
        kw: { main: "kuwo", ikun: "kw", dusiyinyuan: "kw" },
    ...
    function getSongId(platform, songInfo) {
        ...
        switch (platform) {
            ...
            case 'wy': case 'wycloud': case 'wycloudmusic': return info.songmid || info.id || info.songId;
    ...
    async function httpGet(url, params, timeout) {
        ...
        return (await httpRequestWithRetry(full, {method:'GET', timeout})).body;
    }
    ...
    async function juheGetMusicUrl(source, info) {
        ...
        } catch(e) { throw new Error(`juhe: ${e.message}`); }
    }

    // ---------- qorg（修复主接口失败 + 回退自建网易云端点）----------
    async function qorgGetMusicUrl(platform, songInfo, quality) {
        ...
        const ts = Date.now();
        const sign = md5(source + sid + quality + ts);

        try {
            const res = await httpPost(
                CONFIG.QORG_API_URL + '/music/url',
                { source, songId: String(sid), quality: quality || '320k', timestamp: ts, sign },
                15000
            );
            if (res?.code === 200 && res.data?.url) {
                return validateUrl(res.data.url, 'qorg');
            }
            // 失败时输出详细信息以便调试
            console.warn('[qorg] 主接口返回异常:', JSON.stringify(res).slice(0, 300));
            throw new Error(res?.msg || res?.message || '主接口未返回URL');
        } catch (e) {
            console.warn('[qorg] 主接口失败:', e.message, '尝试回退自建网易云接口');
        }

        // 4. 回退：使用自建网易云接口 /song/url
        try {
            const fallbackQualityBra = (() => {
                const q = quality || '320k';
                if (q === '128k') return 128000;
                if (q === '192k') return 192000;
                if (q === '320k') return 320000;
                if (q === 'flac') return 999000;
                return 128000;
            })();
            const fallbackUrl = `${CONFIG.QORG_API_URL}/song/url?id=${sid}&br=${fallbackQualityBra}`;
            const fbRes = await httpGet(fallbackUrl, {}, CONFIG.REQUEST_TIMEOUT);
            if (fbRes?.code === 200 && Array.isArray(fbRes.data) && fbRes.data[0]?.url) {
                const url = validateUrl(fbRes.data[0].url, 'qorg回退(自建网易云)');
                console.log('[qorg] 回退自建网易云成功');
                return url;
            }
            console.warn('[qorg] 回退接口返回异常:', JSON.stringify(fbRes).slice(0, 300));
        } catch (e) {
            console.warn('[qorg] 回退自建网易云也失败:', e.message);
        }

        throw new Error('qorg: 所有端点均获取失败');
    }
    ...
    async function xinghaiMainGetUrl(platform, songId, quality, songInfo) {
        ...
        const res = await httpGet(API_ENDPOINTS.xinghaiMain.base, Object.assign({}, API_ENDPOINTS.xinghaiMain.params, { source, id: String(id), br }));
        if (res?.url) return validateUrl(res.url, '星海');
        if (typeof res === 'string' && HTTP_REGEX.test(res)) return validateUrl(res, '星海');
    ...
    async function xinghaiBackupGetUrl(platform, songId, quality, songInfo) {
        ...
        if (res?.url) return validateUrl(res.url, '星海备');
    ...
    async function initialize() {
        ...
        console.log('[聚合音源 v7.1.2-ultimate-fix-v3.2] 初始化 (qorg 回退增强)');
        startStatsCleanup();
        setupEventListener();
